...
public class ProjetoAssembler implements SimpleRepresentationModelAssembler<Projeto> {
    ...
    public void addLinks(EntityModel<Projeto> resource) {
        ...
        Link liderLink = linkTo(methodOn(FuncionarioControleApi.class).buscaPorId(liderId))
            .withRel("lider")
            .withType("GET");

        Link clienteLink = linkTo(methodOn(ClienteControleApi.class).buscaPorId(clienteId))
            .withRel("cliente")
            .withType("GET");

        Link selfLink = linkTo(methodOn(ProjetoControleApi.class).buscaPorId(id))
            .withSelfRel()
            .withType("GET");
